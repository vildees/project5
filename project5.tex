...
When making an animation of the cluster without the the smoothing function we see that at first most of the particles is pulled towards the center of mass while other is thrown out of the gravitational field. When the particles get close together there is a rapid increase in the particles velocity, and they are spread out. The ones that are not sent out far have to much movement to reach an equilibrium state, and roams around. This is as we could expect. The gravitational force between the particles drags them towards their center of mass. Without the smoothing function the acceleration will increase rapidly as the particles come close together, and thus the kinetic energy increases to be much larger than the potential energy holding the particles together. This can be seen from the figures \ref{kin_pot_notsmooth}. We see that the kinetic energy has a rapid increase, leading to the massive particle ejection. The potential energy will increase (that is get a larger negative value) as the particles move closer together. Soon after the time of the collapse, $\tau_{crunch}$, the potential energy decreases as a result of the particles spreading out from the center of mass. 

We see that the total energy is not conserved. It holds the shape of the kinetic energy, it being some factors larger than the potential energy.  
 
\begin{figure}[H]
	\centering
        \begin{subfigure}[b]{0.6\textwidth}
        \includegraphics[scale=0.5]{kin_usmooth.pdf}
        \caption{\textit{Total kinetic energy for the cluster without smoothing function}}
        
		\end{subfigure}
		
		\begin{subfigure}[b]{0.6\textwidth}
        \includegraphics[scale=0.5]{pot_usmooth.pdf}
        \caption{\textit{Total potential energy for the cluster without smoothing function}}
		\end{subfigure}
		
		\begin{subfigure}[b]{0.6\textwidth}
        \includegraphics[scale=0.5]{ene_usmooth.pdf}
...
From figure \ref{ejectedenergy_usmooth} we see that for the un-modified system most of the ejected energy is sent out in the beginning of the simulation. After a short time the energy ejected becomes constant. From figure \ref{ejectedmass_usmooth} we see that the mass ejection also flattens out for larger $\tau$. But the mass ejection has a steeper curve than the energy for larger time. Thus the particles ejected at a later time will not have quite as high energy as the ones sent out at an early stage. 

 
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.6]{ejectedmass_usmooth.pdf}
	\caption{The mass of the particles that escapes the system as a function of time for the system without a smoothing function}
	\label{ejectedmass_usmooth}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.6]{ejectenergy_usmooth.pdf}
	\caption{The mass of the particles that escapes the system as a function of time for the system without a smoothing function}
	\label{ejectedenergy_usmooth}
\end{figure}


For the cluster system with the modified force we see from \ref{kin_pot} that the kinetic and the potential energy increases towards the collapse at $\tau_{crunch}$. This we can also see from our animation of the cluster. At first the particles, that initially has no velocity, are dragged towards the center by the gravitational force from the other particles. Thus the particles are accelerated and the kinetic energy increases. As the particles come closer together, the gravitational force between them gets stronger as well and the potential energy increases. As we reach the time of the collapse, at $\tau_{crunch}$, many of the particles are ejected. Disregarding the energies of these particles, we find that both the kinetic and potential energy decreases and stabilizes as the system reaches an equilibrium. Both the kinetic and potential energy will fluctuate some, but stays approximately constant.    
...
we calculated the total potential energy divided by the total kinetic energy for a late time step when the system is in equilibrium. We found that this value was approximately $-2$, as it should be.

Figure \ref{ejectedmass} and \ref{ejectedenergy} shows the ejected mass and the ejected energy for a system of $N=100$ particles as a function of $\tau_{crunch}$. We see that these results fits well with our expectations; most of the mass and the energy is ejected soon after the collapse. For time larger than $\tau_{crunch}$ the functions smooths out, corresponding to our system reaching the equilibrium state. 

\begin{figure}
	\centering
	\includegraphics[scale=0.6]{ejected_mass.pdf}
	\caption{The mass of the particles that escapes the system as a function of time}
	\label{ejectedmass}
\end{figure}

\begin{figure}
	\centering
	\includegraphics[scale=0.6]{ejected_energy.pdf}
	\caption{The energy of the particles that escapes the system as a function of time}
	\label{ejectedenergy}
\end{figure}
...
Making a plot of the radial density we see that the density of particles decreases as one moves further away from the center of mass. The gravitational force will be bigger nearer to the center of mass, and thus more particles will be bound in this area. 
...
When we have systems with more particles we see that we have more particles in the inner shells. This can be a reasonable result considering that more particles means larger mass density. The gravitational force holding the particles together will then be larger. 
...
As we can see from the table it seems like the average distance and standard deviation decreases for larger $N$. This is consistent with what we saw from the histogram; for the larger systems we have a bigger concentration in the inner shells, with less spread than for lower $N$. 
 
\begin{figure}
	\centering
	\includegraphics[scale=0.6]{average_distance_N.pdf}
	\caption{The average distance for different N-values}
	\label{Averagedistance}
\end{figure}

To see if our data fit the expression 
\[
n(r) = \frac{n_0}{\left(1 +\left(\frac{r}{r_0}\right)^4\right)}.
\]
we need to find an expression for $r_0$ and $n_0$ for the different $N$-values. To find the values best suited for our histograms we used a curve fitting tool in matlab. For $N=100$ we found the values to be $n_0 = 0.3363$ and $r_0 = 6.432$, and for $N=500$ we found that $n_0 = 64.92$ and $r_0 = 4.994$. When plotting the curves from the expression together with the histograms, we see that the expression fits quite well. It seems to be more precise for the case where $N=100$ than for $N=500$.

\begin{figure}
	\centering
	\includegraphics[scale=0.6]{hist_N100.pdf}
	\caption{Radial distribution of particles}
	\label{Hist100}
\end{figure}

\begin{figure}
	\centering
	\includegraphics[scale=0.6]{hist_N500.pdf}
	\caption{Radial distribution of particles}
	\label{Hist500}
\end{figure}
...
Throughout this project we have found that the Leap-Frog method is more stable than the Runge-Kutta method of fourth order when it comes to time steps and simulating systems that require long time. We therefore concluded that the Leap-Frog method was the best to use when simulating an open cluster, a system for which the stability of
the solution is more important than its short term accuracy. When simulating the cold collapse of a cluster with masses and positions uniformly and randomly distributed, we found that we had to modify the gravitational force to take care of the numerical instabilities that arises when the particles come very close to each other. When adding the modification to the Newtonian force, we saw that the system first contracted towards the center of mass which caused an increase to the energies. At a time $\tau_{crunch}$ the energies got so big that some of the particles got thrown away. After this sudden change, the remaining particles fell into an equilibrium state, ejecting some particles along the way.  
